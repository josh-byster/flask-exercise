...
# Defines the GET, PUT, and DELETE methods for modifying users by ID
@app.route("/users/<user_id>", methods=["GET", "PUT", "DELETE"])
def userById(user_id):
    if request.method == "GET":
        data_for_user = db.getById("users", int(user_id))
        if data_for_user is not None:
            # The user exists, so return its value
            data = {"user": data_for_user}
            return create_response(data)
        else:
            data = {
                "message": "A user with id {0} was not found in the database.".format(
                    user_id
                )
            }
...
# General query of the db to get all the users
def getAllUsers():
    users = db.get("users")
    data = {"users": users}
    return create_response(data)
...
# Helper method to get the users by the team
...
def addUser(user_data):
    ...
    message = "Cannot create new user with missing parameters: got name={0},age={1},team={2} but require that all fields exist.".format(
        user_data.get("name"), user_data.get("age"), user_data.get("team")
    )
    return create_response(message=message, status=422)


# Helper method to update the user in a PUT request
...
# Helper method to delete the user by ID
def deleteUser(user_id):
    ...
    if db.getById("users", int(user_id)) is not None:
        ...
        message = "Could not delete user with id={0}: the user was not found in the database.".format(
            user_id
        )
        return create_response(message=message, status=404)
